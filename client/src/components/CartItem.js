...
export default function CartItem({card, cartQuantity}) {
  const { inventoryId, name, collectorNumber, setName, setCode, rarity, foil, price, image, quantity } = card;
    ...
    <div className="card m-4">
      <div className="row g-0">
        <div className="col-md-4 d-flex justify-content-center align-items-center">
          <img src={`https://c1.scryfall.com/file/scryfall-cards/small${image}`} alt={`${name} #${collectorNumber} ${foil && "(foil)"}`} className="img-fluid px-4 pt-2 " />
        </div>
        <div className="col-md-8">
          <div className="card-body">
            <h5 className="card-title">{name} #{collectorNumber} {foil && "(foil)"}</h5>
